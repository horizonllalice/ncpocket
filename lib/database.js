...
export async function getBudgetSettings() {
  ...
  try {
    ...
    // If we have data, return the first item (since limit(1) returns array)
    if (data && data.length > 0 && !error) {
      return data[0]
    }

    // If no data exists or any error occurs, return default settings
    return {
      needs_percentage: 60,
      wants_percentage: 20,
      savings_percentage: 20,
...
export async function updateBudgetSettings(settings) {
  ...
  try {
    ...
    // If no existing data, include all default values
    if (!existingData || existingData.length === 0) {
      updateData.needs_percentage = settings.needs || 60
      updateData.wants_percentage = settings.wants || 20
      updateData.savings_percentage = settings.savings || 20
      updateData.income_goal = settings.income_goal || 30000
      updateData.needs_goal = settings.needs_goal || 18000
      updateData.wants_goal = settings.wants_goal || 6000
      updateData.savings_goal = settings.savings_goal || 6000
    } else {
      // Start with existing data
      const existing = existingData[0]
      updateData.needs_percentage = settings.needs !== undefined ? settings.needs : existing.needs_percentage
      updateData.wants_percentage = settings.wants !== undefined ? settings.wants : existing.wants_percentage
      updateData.savings_percentage = settings.savings !== undefined ? settings.savings : existing.savings_percentage
      updateData.income_goal = settings.income_goal !== undefined ? settings.income_goal : existing.income_goal
      updateData.needs_goal = settings.needs_goal !== undefined ? settings.needs_goal : existing.needs_goal
      updateData.wants_goal = settings.wants_goal !== undefined ? settings.wants_goal : existing.wants_goal
      updateData.savings_goal = settings.savings_goal !== undefined ? settings.savings_goal : existing.savings_goal
    }
